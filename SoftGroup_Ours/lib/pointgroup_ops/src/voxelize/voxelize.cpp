...
void voxelize_idx(/* int64_t N*4 */ at::Tensor coords, /* int64_t M*4 */ at::Tensor output_coords,
    ...
    Int maxActive = voxelize_inputmap<dimension>(inputSGs, input_map.data<Int>(), voxelizeRuleBook, nActive, coords.data<int64_t>(), coords.size(0), coords.size(1), batchSize, mode);
    ...
    int64_t *oC = output_coords.data<int64_t>();
    voxelize_outputmap<dimension>(coords.data<int64_t>(), oC, oM, &voxelizeRuleBook[1][0], nActive, maxActive);
}


template <Int dimension>
void voxelize_outputmap(int64_t *coords, int64_t *output_coords, Int *output_map, Int *rule, Int nOutputRows, Int maxActive){
...
//output: maxActive
template <Int dimension>
Int voxelize_inputmap(SparseGrids<dimension> &SGs, Int *input_map, RuleBook &rules, Int &nActive, int64_t *coords, Int nInputRows, Int nInputColumns, Int batchSize, Int mode){
    assert(nActive == 0);
    assert(rules.size() == 0);
    assert(SGs.size() == 0);
