...
/root/miniconda3/bin/python tools/prepare_pointnet2_paper_split.py > "$LOGROOT/pointnet2_prepare.log" 2>&1
/root/miniconda3/bin/python train.py --data_path pn2_split --save_path "$RUNROOT/pointnet2_paper/best_model.pth" --num_classes 2 --batch_size 8 --epochs 100 --lr 0.001 --npoints 4096 > "$LOGROOT/pointnet2_train.log" 2>&1
rc=$?
