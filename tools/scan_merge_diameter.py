#!/usr/bin/env python3
"""扫描 merge_min_diameter 参数，找最优碎片合并阈值。

语义推理只做一次，聚类+合并对多个阈值分别执行，输出每个阈值的完整指标。

用法:
  python tools/scan_merge_diameter.py --config configs/pn_watershed_3d.yaml --split test
"""
import sys, os, time, json, glob, warnings, argparse, gc, copy
import numpy as np
...
# 训练时归一化统计量 (与 pointnext_cluster_eval.py 一致)
...
MD_VALUES = [0.15, 0.20, 0.25, 0.30, 0.40, 0.50, 0.60, 0.70, 0.80]
...
def block_inference(points_xyz, points_rgb, model):
    N = len(points_xyz)
    if N <= GPU_MAX:
        prob = _single_infer_prob(points_xyz, points_rgb, model)
...
def postprocess_instances(inst_ng, cab_xyz, cfg, clusterer):
    """对齐 pointnext_cluster_eval.run_one_file 的后处理"""
...
def main():
    ...
    model = load_model()
    print(f'扫描 merge_min_diameter={MD_VALUES}, 文件数={len(files)}', flush=True)

    records_by_md = {md: [] for md in MD_VALUES}
    ...
    for idx, fp in enumerate(files):
        ...
        for md in MD_VALUES:
            ...
            records_by_md[md].append({
                'fname': nm, 'pred_sem': full_sem, 'pred_inst': full_inst,
                'gt_sem': gs, 'gt_inst': gi, 'runtime': 0.0,
            })
        print(f'[{idx+1}/{len(files)}] {nm} 完成 ({n_cab} cabbage pts)', flush=True)
        torch.cuda.empty_cache(); gc.collect()

    print('\n' + '=' * 90)
    print('merge_min_diameter 敏感性扫描 (test, PointNeXt-L + watershed + GIDM)')
    print('=' * 90)
    results = {}
    for md in MD_VALUES:
        res = evaluate_from_predictions(records_by_md[md], iou_thresh=0.5, label=f'md={md}')
        s = res['summary']
        results[md] = s
        print(f"md={md:.2f}m: prec={s['avg_inst_prec']:.3f} rec={s['avg_inst_rec']:.3f} "
              f"f1={s['avg_inst_f1']:.3f} imiou={s['avg_inst_miou']:.3f} "
              f"sem={s['avg_sem_miou']:.3f} mae={s['mae_count']:.2f}")

    out = {'md_values': MD_VALUES, 'results': {str(md): results[md] for md in MD_VALUES}}
    os.makedirs('output', exist_ok=True)
    with open('output/merge_diameter_scan.json', 'w') as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print('\n结果 → output/merge_diameter_scan.json')
